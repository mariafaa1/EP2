...
def imprime_cartela(cartela_de_pontos):
    ...
    for i in range(1, 7):
        ...
        if cartela_de_pontos['regra_simples'][i] != -1:
            print(f"| {i}: {filler}| {cartela_de_pontos['regra_simples'][i]:02} |")
        else:
            print(f"| {i}: {filler}|    |")
    for i in cartela_de_pontos['regra_avancada'].keys():
        filler = " " * (15 - len(str(i)))
        if cartela_de_pontos['regra_avancada'][i] != -1:
            print(f"| {i}: {filler}| {cartela_de_pontos['regra_avancada'][i]:02} |")
        else:
            print(f"| {i}: {filler}|    |")
    print("-"*25)